...
-- FishId to Name mapping (load from external file or HTTP)
local fishIdToName = {}
...
local function SetupFishCaughtListener()
    if fishCaughtRemote and fishCaughtRemote:IsA("RemoteEvent") then
        fishCaughtRemote.OnClientEvent:Connect(function(fishData)
            ...
            -- Extract fish name or fishId
            if type(fishData) == "string" then
                fishName = fishData
            elseif type(fishData) == "table" then
                fishName = fishData.name or fishData.fishName or fishData.Fish or fishData.fishId or "Unknown Fish"
            elseif type(fishData) == "number" then
                fishName = fishData
            end
...
local AutoSell = {
    enabled = true,
    threshold = 2500, -- Default threshold
    ...
    serverThreshold = 1000, -- Server-side threshold
    lastSyncTime = 0,
    syncCooldown = 2, -- 2 seconds between sync attempts
    isThresholdSynced = false,
    syncRetries = 0,
    maxSyncRetries = 3,
    -- Tambahan untuk sinkronisasi slot inventori
    inventoryMax = 5000,
    favoriteCount = 0
}

-- Listener dan fungsi update favoriteCount
local favoriteStateChangedRemote = ResolveRemote("RE/FavoriteStateChanged")

if favoriteStateChangedRemote then
    favoriteStateChangedRemote.OnClientEvent:Connect(function(favoriteList)
        if type(favoriteList) == "table" then
            AutoSell.favoriteCount = #favoriteList
            -- Ambil threshold client (default/manual) dan slot aman
            local threshold_client = AutoSell.threshold or 2500
            local slot_aman = math.max(1, (AutoSell.inventoryMax or 5000) - AutoSell.favoriteCount - 1)
            -- Threshold final adalah nilai minimum keduanya
            AutoSell.threshold = math.min(threshold_client, slot_aman)
        end
    end)
end

local function GetAvailableInventorySlot()
    return AutoSell.inventoryMax - (AutoSell.favoriteCount or 0)
end
-- ====================================================================
...
local function BuildUI()
    ...
    local function updatePlayerList(filter)
        ...
        if playerScrollFrame.CanvasSize.Y.Offset ~= yPos then
    ...
    -- Function to add a section and update layoutY
    local function addSection(section)
        section.Position = UDim2.new(0, 5, 0, layoutY)
        layoutY = layoutY + section.Size.Y.Offset + sectionPadding
        return section
    end
    ...
    if success and SettingTabFeature and SettingTabFeature.CreateUI then
        ...
        layoutY = layoutY + 180 + sectionPadding
    end
    ...
    weatherSection.Size = UDim2.new(1, -10, 0, 150) -- Adjusted height to fit the new button
    ...
    buyAllBtn.Size = UDim2.new(1, -20, 0, 25)
    buyAllBtn.Position = UDim2.new(0, 10, 0, 120)
    ...
    -- Random Location Feature Section (load dari raw github)
    local success, RandomLocationFeature = pcall(function()
        local url = "https://raw.githubusercontent.com/donitono/full/refs/heads/main/RandomLocationFeature.lua"
        local source = game:HttpGet(url)
        local chunk = loadstring(source)
        return chunk and chunk()
    end)
    if success and RandomLocationFeature and RandomLocationFeature.CreateUI then
        local randomLocSection = RandomLocationFeature.CreateUI(featureScrollFrame)
        addSection(randomLocSection)
    else
        warn("RandomLocationFeature gagal dimuat: " .. tostring(RandomLocationFeature))
    end

    -- Set canvas size for feature scroll frame
    featureScrollFrame.CanvasSize = UDim2.new(0, 0, 0, layoutY)
    ...
    -- Load FishDataDashboard.lua from GitHub raw link and add fish data section
    local url = "https://raw.githubusercontent.com/donitono/full/refs/heads/main/FishDataDashboard.lua"
    local source = game:HttpGet(url)
    local chunk, err = loadstring(source)
    if chunk then
        chunk()
        if CreateFishDataSection then
            CreateFishDataSection(dashboardScrollFrame)
        end
    else
        warn("Gagal load FishDataDashboard.lua: " .. tostring(err))
    end

    -- Set canvas size for dashboard scroll
    dashboardScrollFrame.CanvasSize = UDim2.new(0, 0, 0, 900) -- increased for extra section
...
_G.ModernAutoFish = {
    ...
    SetSellThreshold = function(threshold) 
        if threshold > 0 and threshold <= 8000 then 
