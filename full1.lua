...
-- Add Header Buttons (Floating Mode & Close)
task.spawn(function()
    ...
    if rayfieldGui then
        ...
        if main then
            local topbar = main:FindFirstChild("Topbar") or main:FindFirstChild("TitleBar") or main:GetChildren()[1]
            if topbar then
                -- Create Floating Mode Button (-)
                local floatingBtn = Instance.new("TextButton")
                floatingBtn.Name = "FloatingModeBtn"
                floatingBtn.Size = UDim2.new(0, 25, 0, 25)
                floatingBtn.Position = UDim2.new(1, -60, 0, 5)
                floatingBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
                floatingBtn.BorderSizePixel = 0
                floatingBtn.Text = "–"
                floatingBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
                floatingBtn.TextSize = isMobile and 8 or 10
                floatingBtn.Font = Enum.Font.GothamBold
                floatingBtn.Parent = topbar
                
                -- Add corner radius
                local floatingCorner = Instance.new("UICorner")
                floatingCorner.CornerRadius = UDim.new(0, 4)
                ...
                closeBtn.TextSize = isMobile and 8 or 10
                ...
                -- Store references globally for floating button access
                _G.XSANHeaderButtons = {
                    floating = floatingBtn,
                    close = closeBtn,
                    isUIVisible = true
                }
                
                -- Floating Mode Button Click
                floatingBtn.MouseButton1Click:Connect(function()
                    _G.XSANHeaderButtons.isUIVisible = false
                    -- Hide main UI
                    if main then
                        main.Visible = false
                        NotifyInfo("UI Mode", "🎈 Floating mode aktif!\n\n💡 Gunakan floating button untuk menampilkan UI kembali")
                    end
                end)
                ...
                closeBtn.MouseButton1Click:Connect(function()
                    -- Show confirmation
                    local confirmClose = true
                    if confirmClose then
                        NotifyInfo("Script Exit", "👋 XSAN Fish It Pro Ultimate ditutup!\n\n✅ Semua fitur dihentikan dengan aman\n💝 Terima kasih telah menggunakan script kami!")
                        
                        -- Reset all features before closing
                        pcall(function()
                        ...
                        _G.XSANHeaderButtons = nil
                        _G.XSANReset = nil
                        _G.XSAN_FloatingButton = nil
                    end
                end)
                
                -- Add hover effects
                floatingBtn.MouseEnter:Connect(function()
                    floatingBtn.BackgroundColor3 = Color3.fromRGB(70, 70, 70)
                end)
                ...
                print("XSAN: Header buttons created successfully!")
            else
                print("XSAN: Could not find topbar for header buttons")
            end
        else
            print("XSAN: Could not find main UI for header buttons")
        end
    else
        print("XSAN: Could not find Rayfield GUI for header buttons")
    end
end)
                floatingBtn.Name = "FloatingModeBtn"
                floatingBtn.Size = UDim2.new(0, 25, 0, 25)
                ...
                floatingBtn.Text = "⚊"
                floatingBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
                floatingBtn.TextSize = isMobile and 12 or 14
                floatingBtn.Font = Enum.Font.SourceSansBold
                floatingBtn.BorderSizePixel = 0
                floatingBtn.ZIndex = topbar.ZIndex + 1
                
                -- Add corner radius
                local corner1 = Instance.new("UICorner")
                corner1.CornerRadius = UDim.new(0, 4)
                corner1.Parent = floatingBtn
                ...
                -- Parent buttons to topbar
                floatingBtn.Parent = topbar
                closeBtn.Parent = topbar
                
                -- Floating Mode Button Functionality
                floatingBtn.MouseButton1Click:Connect(function()
                    -- Toggle to floating button mode
                    main.Visible = false
                    NotifyInfo("UI Mode", "🎈 Switched to floating mode!\n\n💡 Use floating button or press 'H' to show UI again")
                    
                    -- Visual feedback
                    local originalColor = floatingBtn.BackgroundColor3
                    floatingBtn.BackgroundColor3 = Color3.fromRGB(150, 200, 255)
                    task.wait(0.1)
                    floatingBtn.BackgroundColor3 = originalColor
                end)
                
                -- Close Button Functionality
                closeBtn.MouseButton1Click:Connect(function()
                    -- Show confirmation
                    NotifyInfo("XSAN Script", "🛑 Closing XSAN Fish It Pro Ultimate...\n\n✅ All features safely disabled\n👋 Thank you for using XSAN!")
                    
                    -- Visual feedback
                    local originalColor = closeBtn.BackgroundColor3
                    ...
                    -- Clean up global variables
                    _G.XSANReset = nil
                    _G.XSANSavedPosition = nil
                    
                    print("XSAN: Script closed successfully!")
                end)
                
                -- Hover effects for buttons
                floatingBtn.MouseEnter:Connect(function()
                    TweenService:Create(floatingBtn, TweenInfo.new(0.2), {
...
task.spawn(function()
    task.wait(2) -- Wait for UI and header buttons to fully load
    ...
    -- Get reference to main UI and header buttons
    local rayfieldGui = game.Players.LocalPlayer.PlayerGui:FindFirstChild("RayfieldLibrary") or game.CoreGui:FindFirstChild("RayfieldLibrary")
    local mainUI = rayfieldGui and rayfieldGui:FindFirstChild("Main")
    local isUIVisible = true
    
    -- Function to update UI visibility state
    local function updateUIVisibility(visible)
        isUIVisible = visible
        if _G.XSANHeaderButtons then
            _G.XSANHeaderButtons.isUIVisible = visible
        end
    ...
    -- Store global reference for header buttons
    _G.XSAN_FloatingButton = {
        show = function() updateUIVisibility(true) end,
        hide = function() updateUIVisibility(false) end,
        toggle = function() updateUIVisibility(not isUIVisible) end
    }
    ...
    FloatingButton.TextSize = isMobile and 16 or 18
    ...
    -- Toggle UI function (integrated with header buttons)
    local function toggleUI()
        updateUIVisibility(not isUIVisible)
        ...
        if isUIVisible then
            NotifyInfo("UI Toggle", "✅ UI ditampilkan!")
        else
            NotifyInfo("UI Toggle", "🎈 UI mode floating! Gunakan floating button untuk menampilkan kembali")
        end
    end
    ...
    print("XSAN: - Press 'F' key for auto face water")
end)
...
local function TeleportToPlayer(targetPlayerName)
    ...
    if not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        NotifyError("Player TP", "❌ Character Anda tidak ditemukan!")
        return
    end
    ...
    local success = pcall(function()
        ...
        -- Simple direct teleport like main16.lua - most reliable method
        local targetPosition = targetHRP.CFrame + Vector3.new(
            math.random(-5, 5), -- Random offset to avoid overlap
            2, -- Slightly above to avoid falling through ground
            math.random(-5, 5)
        )
        
        -- Direct CFrame assignment - simple and effective
...
local function SafeTeleport(targetCFrame, locationName)
    local success = pcall(function()
        ...
        -- Simple direct teleport like main16.lua - most reliable
        LocalPlayer.Character.HumanoidRootPart.CFrame = targetCFrame
        
        -- Auto face water after teleporting (optional delay)
        task.wait(0.1) -- Small delay to ensure teleport is complete
        AutoFaceWater(LocalPlayer.Character.HumanoidRootPart, locationName)
    ...
    if not success then
        NotifyError("Teleport", "❌ Teleport failed to: " .. locationName)
    end
end
...
UtilityTab:CreateButton({ 
    ...
    Callback = CreateSafeCallback(function()
        ...
        if not success then
            NotifyError("Rejoin", "❌ Failed to rejoin server!")
        end
    end, "rejoin_server")
})
...
SettingTab:CreateParagraph({
    Title = "🏃 Speed & Jump Controls",
    Content = "Kontrol kecepatan jalan dan tinggi lompatan karakter"
})
...
SettingTab:CreateSlider({
    ...
    Callback = CreateSafeCallback(function(value)
        ...
        if walkspeedEnabled then
            setWalkSpeed(value)
            NotifyInfo("Walk Speed", "🚶 Kecepatan jalan diubah ke: " .. value)
        else
            NotifyInfo("Walk Speed", "🚶 Kecepatan diatur ke " .. value .. ". Aktifkan untuk menerapkan.")
        end
...
SettingTab:CreateToggle({
    ...
    Callback = CreateSafeCallback(function(value)
        ...
        if value then
            ...
            NotifySuccess("Walk Speed", "🚶 Walk speed diaktifkan: " .. currentWalkspeed)
        else
            resetWalkSpeed()
            NotifyInfo("Walk Speed", "🚶 Walk speed dimatikan (kembali ke default)")
        end
...
SettingTab:CreateSlider({
    ...
    Callback = CreateSafeCallback(function(value)
        ...
        if not unlimitedJumpEnabled then
            setJumpHeight(value)
            NotifyInfo("Jump Height", "🦘 Tinggi lompatan: " .. value .. "\n💡 Aktifkan Unlimited Jump untuk lompatan tak terbatas!")
        else
            setJumpHeight(value)
            NotifyInfo("Jump Height", "🦘 Tinggi lompatan diubah: " .. value .. " (Mode tak terbatas)")
        end
...
-- Quick Action Buttons for Speed & Jump
SettingTab:CreateButton({
    Name = "⚡ Quick Speed: 45",
    Callback = CreateSafeCallback(function()
        currentWalkspeed = 45
        ...
        NotifySuccess("Quick Speed", "⚡ Walk speed diatur ke 45!")
    end, "quick_speed_45")
})
...
SettingTab:CreateButton({
    ...
    Callback = CreateSafeCallback(function()
        ...
        NotifySuccess("Super Jump", "🎯 Super high jump diaktifkan!\n\nTinggi: 75\n💡 Aktifkan Unlimited Jump untuk lompatan tak terbatas!")
    end, "super_jump")
})
...
SettingTab:CreateButton({
    ...
    Callback = CreateSafeCallback(function()
        ...
        NotifySuccess("Reset", "🔄 Speed & Jump direset ke default!\n\n🚶 Walk Speed: " .. defaultWalkspeed .. "\n🦘 Jump Height: " .. defaultJumpHeight)
    end, "reset_speed_jump")
})
...
SettingTab:CreateButton({
    ...
    Callback = CreateSafeCallback(function()
        ...
        status = status .. "🏃 Speed & Jump:\n"
        status = status .. "   🚶 Walk Speed: " .. (walkspeedEnabled and ("✅ " .. currentWalkspeed) or "❌ OFF") .. "\n"
        status = status .. "   🦘 Jump Height: " .. currentJumpHeight .. "\n"
        status = status .. "   🚀 Unlimited Jump: " .. (unlimitedJumpEnabled and "✅ ON" or "❌ OFF") .. "\n\n"
