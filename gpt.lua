...
-- ========== GLOBAL CHAT SCAN & AUTO JOIN BUTTONS ==========
local recentGlobalPlayers = {}
local maxRecentPlayers = 10

-- Fungsi untuk menambah nama player ke list (tanpa duplikat, max 10)
local function AddRecentGlobalPlayer(name)
    if not name or name == "" then return end
    for _, n in ipairs(recentGlobalPlayers) do
        if n == name then return end
    end
    table.insert(recentGlobalPlayers, 1, name)
    if #recentGlobalPlayers > maxRecentPlayers then
        table.remove(recentGlobalPlayers)
    end
end

-- Hook ke RemoteEvent notifikasi global (coba beberapa kemungkinan)
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer
local net = ReplicatedStorage:FindFirstChild("Packages") and ReplicatedStorage.Packages:FindFirstChild("_Index") and ReplicatedStorage.Packages._Index:FindFirstChild("sleitnick_net@0.2.0") and ReplicatedStorage.Packages._Index["sleitnick_net@0.2.0"].net
local displaySystemMsg = net and net.RE and net.RE:FindFirstChild("DisplaySystemMessage")
local textNotif = net and net.RE and net.RE:FindFirstChild("TextNotification")

local function scanGlobalChat(msg)
    -- Cek pola notifikasi global rare fish
    local name = string.match(msg, "%[Global%]: ([%w_]+) obtained")
    if not name then
        name = string.match(msg, "%[Global%]: ([%w_]+) mendapatkan") -- versi indo
    end
    if name then
        AddRecentGlobalPlayer(name)
        -- Optional: NotifyInfo("Global Scan", "Player ditemukan: "..name)
    end
end

if displaySystemMsg then
    displaySystemMsg.OnClientEvent:Connect(scanGlobalChat)
end
if textNotif then
    textNotif.OnClientEvent:Connect(scanGlobalChat)
end

-- UI: Section recent global players (auto update)
            ...
            Callback = CreateSafeCallback(function()
                ...
                spawn(function()
                    while true do
                        ...
                        if tryCount > 10 then break end
                    end
                    if not found then
                        NotifyError("Join Server", "❌ Player tidak ditemukan di server publik manapun!")
                    end
                end)
...
-- Input dan tombol untuk join ke server player lain (berdasarkan nama)
local joinTargetName = ""

TeleportTab:CreateInput({
    Name = "📝 Masukkan Nama Player (Join Server)",
...
TeleportTab:CreateButton({
    Name = "🌍 Join Server Player Ini",
    Callback = CreateSafeCallback(function()
        if not joinTargetName or joinTargetName == "" then
            NotifyError("Join Server", "❌ Masukkan nama player terlebih dahulu!")
            return
        end
        NotifyInfo("Join Server", "🔍 Mencari server untuk player: " .. joinTargetName .. " ...")
        local HttpService = game:GetService("HttpService")
        local TeleportService = game:GetService("TeleportService")
        local PlaceId = game.PlaceId
        local found = false
        ...
        spawn(function()
            while true do
                ...
                if success and response and response.data then
                    for _, server in ipairs(response.data) do
                        if server.playing and server.playing > 0 and server.id then
                            -- Cek nama player di server ini
                            if server.players then
                                for _, player in ipairs(server.players) do
                                    if player and player.username and string.lower(player.username) == string.lower(joinTargetName) then
                                        found = true
                                        NotifySuccess("Join Server", "✅ Server ditemukan! Join ke server " .. joinTargetName .. " ...")
                                        TeleportService:TeleportToPlaceInstance(PlaceId, server.id, game.Players.LocalPlayer)
                                        return
                                    end
                                end
                ...
                if tryCount > 10 then break end -- Batas pencarian
            end
            if not found then
                NotifyError("Join Server", "❌ Player tidak ditemukan di server publik manapun!")
            end
        end)
    end, "join_player_server")
})
